...
-- Run with: nvim --clean --headless -c "luafile test.lua"
...
local function test_ui_creation()
  ...
  assert(
    type(ui.jump_to_source_location) == "function",
    "jump_to_source_location is not a function"
  )
  assert(type(ui.stage_current_hunk) == "function", "stage_current_hunk is not a function")
  assert(
    type(ui.toggle_current_hunk_stage) == "function",
    "toggle_current_hunk_stage is not a function"
  )
  ...
  assert(
    ...
    "Expected deleted line to jump to next source line"
  )
